...
class XYCollectionController: ASViewController<ASDisplayNode>,ASCollectionDataSource,ASCollectionDelegate{
    
    // MARK: - Private properties
    let collectionNode:XYCollectionNode
    
    override init(node: ASDisplayNode) {
        collectionNode = XYCollectionNode.init(layout: nil, header: nil, footer: nil)
    ...
    func shouldBatchFetch(for collectionNode: ASCollectionNode) -> Bool {
        if let _collectionNode = collectionNode as? XYCollectionNode{
