...
protocol XYRefreshControlHeaderAble {
    var header: XYHeaderRefreshControl? {get}
}
...
protocol XYRefreshControlFooterAble {
    var footer: XYFooterRefreshControl? {get}
}
...
// MARK: - 通用刷新控件基类
class XYRefreshControl:ASDisplayNode {
    ...
    var beginRefreshCallback:((_ control:XYRefreshControl)->Void)?
    // End Refresh Callback
    var endRefreshCallback:((_ control:XYRefreshControl)->Void)?
    // Height
    var height: CGFloat = 44.0

    // Init instance
    init(beginRefreshCallback:((_ control:XYRefreshControl)->Void)?,
         endRefreshCallback:((_ control:XYRefreshControl)->Void)?) {
        super.init()
...
// MARK: - 下拉刷新基础控件
class XYHeaderRefreshControl:XYRefreshControl {
...
// MARK: - 上拉刷新基础控件
class XYFooterRefreshControl:XYRefreshControl {
